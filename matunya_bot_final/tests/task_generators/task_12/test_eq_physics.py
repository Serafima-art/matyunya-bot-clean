...
# Теперь мы смотрим на 'eq_physics'
from matunya_bot_final.task_generators.task_12.generators import eq_physics as gen_module
from matunya_bot_final.task_generators.task_12.validators import eq_physics as val_module


# --- 2. Создаем локальные MAP'ы для конкретного модуля ---
def _create_local_maps() -> Tuple[Dict[str, Callable], Dict[str, Callable]]:
    """
    Создает словари генераторов и валидаторов только для модуля eq_physics.
    """
    gen_map = {}
    val_map = {}
...
def test_generator_vs_validator_eq_physics():
    """Проверяет совместимость генераторов и валидаторов из eq_physics."""
    errors = []
    if not LOCAL_GENERATOR_MAP:
        pytest.skip("В модуле eq_physics нет генераторов для теста.")
    ...
    assert not errors, f"Ошибки валидации для eq_physics:\n{errors}"

def test_subtype_keys_match_eq_physics():
    """Проверяет, что у каждого генератора в eq_physics есть свой валидатор."""
    gen_keys = set(LOCAL_GENERATOR_MAP.keys())
    val_keys = set(LOCAL_VALIDATOR_MAP.keys())

    assert gen_keys == val_keys, (
        f"Несоответствие ключей в eq_physics:\n"
        f"Только в генераторе: {gen_keys - val_keys}\n"
        f"Только в валидаторе: {val_keys - gen_keys}"
    )

def test_constants_usage_eq_physics():
    """Проверяет использование констант в eq_physics."""
...
if __name__ == "__main__":
    ...
    print("✅ Все проверки для eq_physics запущены.")