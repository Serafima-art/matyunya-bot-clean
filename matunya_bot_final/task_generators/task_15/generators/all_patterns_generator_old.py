...
Интеграция всех паттернов согласно требованиям ОГЭ-2026
"""
...
class AllPatternsGenerator:
    ...
    def _generate_task_data(self, subtype: str, difficulty: str) -> Dict[str, Any]:
        ...
        if subtype == "right_triangle_angles_sum":
            ...
            area = 0.5 * side_a * side_b * (3**0.5/2 if angle_c == 60 else 1 if angle_c == 90 else 0.707 if angle_c == 45 else 0.5)
            ...
            if trig_ratio == 'sin':
                unknown_side = known_side / random.choice([0.5, 0.707, 0.866])  # sin30, sin45, sin60
            elif trig_ratio == 'cos':
                unknown_side = known_side / random.choice([0.866, 0.707, 0.5])  # cos30, cos45, cos60
            else:  # tg
                unknown_side = known_side * random.choice([0.577, 1, 1.732])  # tg30, tg45, tg60
    ...
    def _generate_task_text(self, subtype: str, data: Dict[str, Any]) -> str:
        ...
        templates = {
            "right_triangle_angles_sum": f"В прямоугольном треугольнике ABC угол A = {data['known_angle']}°. Найдите угол B.",
            
            "isosceles_triangle_angles": f"В равнобедренном треугольнике ABC AB = BC. Угол при основании AC равен {data['base_angle']}°. Найдите угол при вершине B.",
            
            "triangle_external_angle": f"В треугольнике ABC угол A = {data['internal_angle1']}°, угол B = {data['internal_angle2']}°. Найдите внешний угол при вершине C.",
            
            "pythagoras_find_leg": f"В прямоугольном треугольнике известны катет = {data['known_leg']} и гипотенуза = {data['hypotenuse']}. Найдите другой катет.",
            
            "pythagoras_find_hypotenuse": f"В прямоугольном треугольнике катеты равны {data['leg1']} и {data['leg2']}. Найдите гипотенузу.",
            
            "find_cos_sin_tg_from_sides": f"В прямоугольном треугольнике катеты равны {data['leg_a']} и {data['leg_b']}, гипотенуза ≈ {data['hypotenuse']}. Найдите {data['trig_function']} угла при катете {data['leg_a']}.",
            
            "find_side_from_trig_ratio": f"В прямоугольном треугольнике известно {data['trig_ratio']}({data['angle']}°) = {data['known_side']}/{data.get('unknown_side', '?')} и известна сторона = {data['known_side']}. Найдите неизвестную сторону.",
            
            "right_triangle_median_to_hypotenuse": f"В прямоугольном треугольнике медиана к гипотенузе равна {data['median']}. Найдите гипотенузу (медиана = гипотенуза ÷ 2).",
            
            "equilateral_height_to_side": f"В равностороннем треугольнике высота равна {data['height']}. Найдите сторону треугольника.",
            
            "equilateral_side_to_height": f"В равностороннем треугольнике сторона равна {data['side']}. Найдите высоту треугольника.",
            
            "triangle_area_by_sin": f"В треугольнике две стороны равны {data['side_a']} и {data['side_b']}, угол между ними {data['angle_c']}°. Найдите площадь треугольника.",
            
            "triangle_area_by_dividing_point": f"На стороне треугольника отмечена точка, делящая её в отношении {data['ratio']}:1. Площадь большого треугольника = {data['area_large']}. Найдите площадь меньшего треугольника.",
            
            "triangle_area_by_parallel_line": f"Через вершину треугольника проведена прямая, параллельная стороне. Отношение сторон = {data['ratio']}. Площадь большого треугольника = {data['area_large']}. Найдите площадь малого треугольника.",
            
            "triangle_area_by_midpoints": f"Точки — середины сторон треугольника. Площадь большого треугольника = {data['area_large']}. Найдите площадь малого треугольника (1/4 от большого).",
            
            "angle_bisector_find_half_angle": f"В треугольнике проведена биссектриса угла {data['full_angle']}°. Найдите угол между биссектрисой и стороной (половину данного угла).",
            
            "cosine_law_find_cos": f"В треугольнике стороны равны a={data['side_a']}, b={data['side_b']}, c={data['side_c']}. Найдите cos угла A."
        }
        
        return templates.get(subtype, f"Задача подтипа '{subtype}' - данные: {data}")
