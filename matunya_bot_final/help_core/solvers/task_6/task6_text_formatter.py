"""
task6_text_formatter.py — единый форматер и валидатор для всех подтипов задания №6.

Цели:
• устранить деление на ноль (в том числе «приобретённое»);
• очистить выражения от кракозябр, странных минусов, неразрывных пробелов;
• обернуть отрицательные множители и делители в скобки;
• унифицировать запись (запятые, пробелы, точки, умножение «·»);
• вернуть None, если выражение некорректно — генератор тогда делает перегенерацию.
"""

from __future__ import annotations
import re
from decimal import Decimal, InvalidOperation
import random
from fractions import Fraction

# --- 🔧 Базовые регулярки ---
_NEG_AFTER_OP_RE = re.compile(r'([·*/:])\s*(−|-)\s*(\d+(?:[.,]\d+)?)')
_BAD_TRAILING_OP_RE = re.compile(r'[\+\-\*/:]$')
_NON_BREAK_SPACE_RE = re.compile(r'[\u00A0\u202F]')
_UNARY_MINUS_SPACE_RE = re.compile(r'(?:(?<=\()|^)\s*(?:−|–|—|-)\s+(\d+(?:[.,]\d+)?)')
_WRAP_NEG_AFTER_PM_RE = re.compile(r'([+\-])\s*−\s*(\d+(?:[.,]\d+)?)')
_ALLOWED = "0123456789 +-−·:/()"

# --- 🔍 Нормализация выражения ---
def normalize_expression(expr: str) -> str:
    """
    Приводит выражение к стандартной, идеально отформатированной форме.
    Финальная, надёжная версия.
    """
    if not expr:
        return ""

    s = expr

    # 1. Базовая стандартизация символов
    s = _NON_BREAK_SPACE_RE.sub(" ", s)
    s = s.replace("−", "-").replace("–", "-").replace("—", "-")
    s = s.replace("×", "*").replace("·", "*")  # временно приводим все умножения к *
    s = s.replace(":", "/")                    # временно приводим все деления к /

    # 2. Заменяем десятичную точку на запятую
    s = re.sub(r"(?<=\d)\.(?=\d)", ",", s)

    # 3. Простая и надёжная логика формирования пробелов
    s = re.sub(r'\s*/\s*', '/', s)
    for op in ['+', '-', '*', '/']:
        s = s.replace(op, f' {op} ')

    # 4. Возвращаем красивые символы умножения и деления
    s = re.sub(r'\*', ' · ', s)
    s = re.sub(r'/', ' : ', s)

    # 5. Убираем лишние пробелы и используем красивую точку умножения
    s = re.sub(r'\s+', ' ', s).strip()
    s = s.replace(" · ", " ⋅ ")

    return s


def fix_negative_after_operators(expr: str) -> str:
    """Обернуть отрицательные числа после знаков умножения или деления в скобки."""
    return _NEG_AFTER_OP_RE.sub(r'\1(−\3)', expr)

def fix_unary_minus_spacing(expr: str) -> str:
    """
    Убирает пробел после унарного минуса в начале выражения
    или сразу после открывающей скобки: '− 4,5' -> '−4,5', '( − 2,1)' -> '(−2,1)'.
    """
    return _UNARY_MINUS_SPACE_RE.sub(r'−\1', expr)


def wrap_negative_after_plus_minus(expr: str) -> str:
    """
    Оборачивает отрицательное слагаемое после +/− в скобки:
    '7,7 + −6,3' -> '7,7 + (−6,3)' ; 'a − −2,1' -> 'a − (−2,1)'.
    """
    return _WRAP_NEG_AFTER_PM_RE.sub(r'\1 (−\2)', expr)


def validate_expression(expr: str) -> bool:
    """Проверяет, можно ли безопасно вычислить выражение."""
    if not expr or _BAD_TRAILING_OP_RE.search(expr):
        return False

    expr_eval = expr.replace("·", "*").replace(",", ".")
    try:
        val = Decimal(str(eval(expr_eval)))
        if val.is_nan() or val.is_infinite():
            return False
    except (InvalidOperation, ZeroDivisionError, SyntaxError, NameError):
        return False
    except Exception:
        return False
    return True


def prepare_expression(src: str) -> str | None:
    """
    Приводит исходное выражение к безопасному и чистому виду.
    Возвращает None, если выражение некорректно.
    """
    if src is None:
        return None

    # Убираем неразрывные пробелы и лишние символы
    s = (src.replace("\u00a0", " ")
             .replace("\u202f", " ")
             .strip())

    # Фильтруем только допустимые символы
    s = "".join(ch for ch in s if ch in _ALLOWED)

    # Нормализуем знаки и пробелы
    s = s.replace("--", "+")
    s = re.sub(r"\s+", " ", s).strip()
    s = s.replace("-", "−")  # длинное тире

    # Проверяем корректность
    if not s or not re.search(r"[0-9]", s):
        return None
    if not re.search(r"[/:·+\-−()]", s):
        return None
    if re.search(r"\(\s*\)", s):
        return None

    return s


def _fmt(x: float) -> str:
    """Форматирует числа для выражений (замена точки на запятую, скобки для отрицательных)."""
    s = f"{x:.2f}".replace(".", ",")
    s = s.rstrip("0").rstrip(",")
    return f"({s})" if x < 0 else s


def _fmt_answer(x: float, use_comma: bool = False) -> str:
    """
    Форматирует ответ без скобок.
    По умолчанию — с точкой, максимум два знака.
    Если use_comma=True, заменяет точку на запятую.
    """
    try:
        val = float(x)
        s = f"{val:.2f}"
    except Exception:
        s = str(x)

    s = s.rstrip("0").rstrip(".")
    if s == "-0":
        s = "0"

    if use_comma:
        if "." in s:
            s = s.replace(".", ",")
        if "," not in s and any(ch.isdigit() for ch in s):
            s = f"{s},0"
    else:
        if "." not in s and any(ch.isdigit() for ch in s):
            if "," in str(x):
                s = s.replace(",", ".")
    return s


# --- 🌟 Единый централизованный форматер для задания 6 --- #

def normalize_for_display(expr: str, subtype: str = "") -> str:
    """
    Универсальный форматтер для всего задания №6.
    Делает выражение читаемым и единообразным.
    Используется во всех подтипах:
      - common_fractions
      - decimal_fractions
      - mixed_fractions
      - powers

    Правила:
      • decimal_fractions → деление двоеточием (10 : 5)
      • common_fractions и mixed_fractions → сохраняют /
      • степени и корни форматируются как обычно
      • отрицательные числа после знаков оборачиваются в скобки
    """
    if not expr:
        return ""

    s = str(expr).strip()

    # 1. Нормализация десятичных чисел (точка → запятая)
    s = s.replace(".", ",")
    s = re.sub(r"\s+", " ", s)

    # 2. Оборачиваем отрицательные числа после знаков (·, :, /)
    s = fix_negative_after_operators(s)

    # 3. Меняем короткий минус на длинный (ГОСТ)
    s = s.replace("-", "−")

    # 4. Приводим знак умножения к единому символу
    s = s.replace("·", "⋅")

    # 5. Условная замена деления:
    # Только для десятичных дробей используем двоеточие
    if subtype == "decimal_fractions":
        s = s.replace("/", " : ")

    # 6. Финальная чистка пробелов
    s = re.sub(r"\s+", " ", s).strip()

    return s

__all__ = [
    "normalize_expression",
    "fix_negative_after_operators",
    "fix_unary_minus_spacing",
    "wrap_negative_after_plus_minus",
    "validate_expression",
    "prepare_expression",
    "_fmt",
    "_fmt_answer",
]
