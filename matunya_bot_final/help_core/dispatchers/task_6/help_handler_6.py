...
async def handle_task_6_help(
    ...
    """Обрабатывает запрос помощи по задачам №6 (все подтипы)."""
    try:
        ...
        try:
            humanized_solution = humanize_solution_6(solution_core)
            humanized_solution = clean_html_tags(humanized_solution)
        except Exception as exc:  # pragma: no cover
            logger.error("[Help6] Ошибка гуманизации: %s", exc)
