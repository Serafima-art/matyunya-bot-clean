...
# Этот файл НЕ решает задачу.
# Он проверяет КОРРЕКТНОСТЬ УСЛОВИЯ задания 15:
# есть ли достаточно данных, чтобы задачу ВООБЩЕ можно было решить
# (можно ли вычислить коэффициент подобия k и допустим ли тип искомого).
#
# Используется как логический фильтр ДО валидатора и решателя.
#
# Применяется для паттернов задания 15, связанных с параллельной прямой:
# - triangle_area_by_parallel_line
# - triangle_ratio_by_parallel_line
# - triangle_side_by_parallel_line
#
# Если проверка не пройдена — задача считается некорректной и отбрасывается.
